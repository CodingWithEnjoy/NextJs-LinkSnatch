...
export default function Home() {
  ...
  function searchLinks() {
    ...
    const filteredLinks =
      links?.filter((link) =>
        link.title?.toLowerCase().includes(url.toLowerCase())
      ) ?? [];
    setLinks(filteredLinks);
  }
    ...
    <>
      ...
      <div class="container mx-auto px-4 flex justify-center mt-10">
        <div class="relative lg:w-1/2 w-full">
          <input
            type="text"
            className={`w-full border bg-gray-100 border-slate-300 dark:border-slate-600 outline-none focus:outline-none focus:ring lg:flex items-center text-sm sm:text-lg md:text-lg leading-6 text-slate-600 dark:text-slate-300 rounded-2xl shadow-sm py-4 px-4 pr-3 hover:ring-slate-300 dark:bg-slate-800 dark:highlight-white/5 dark:hover:bg-slate-700 ${
              hasValidUrl ? "focus:ring-green-300 ring-green-300" : ""
            }`}
          ...
          <button
            type="button"
            className={`absolute right-3 top-4 sm:top-5 md:top-5 ${
              hasValidUrl && !showSpinner ? "show" : "hidden"
            }`}
            onClick={() => saveLink()}
          >
            <svg
              ...
              strokeWidth="1.5"
              stroke="currentColor"
              class="w-6 h-6 stroke-gray-500 hover:stroke-gray-700"
            >
              <path
                strokeLinecap="round"
          ...
          {showSpinner && (
            <div
              class="animate-spin inline-block w-6 h-6 border-[3px] border-current border-t-transparent text-violet-400 rounded-full absolute right-3 top-4 sm:top-5 md:top-5"
              role="status"
              aria-label="loading"
            >
      ...
      {links?.length > 0 && <Links links={links} deleteLink={deleteLink} />}

      {links.length === 0 && <Placeholder />}
    </>
  );
}
